...
- `package.json` e `package-lock.json`: script e dipendenze npm. `cors` è dichiarato ma non utilizzato dal server.
...
Usare Node.js 22, in linea con il container, e npm con il lockfile esistente. Anteporre `rtk` ai comandi shell come richiesto da `@/home/marcuson/.codex/RTK.md`.
...
`start` avvia il server; `watch` lo avvia con il riavvio automatico. Le variabili lette sono `PORT` (default `3000`) e `LOG_LEVEL` (default `info`). Non è presente un caricatore di file `.env`: passare le variabili nell'ambiente del processo.
